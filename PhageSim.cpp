...
int main()
{
	...
	for (i = starting_step; i < simulation_steps; i++)
	{
		...
		Randomly choose qd * total_phage_size number of phages and kill each one.*/
		if (binomial == false)
		{
		...
		//if coinfection is not allowed, infection_mode = 0 or 1
		if (infection_mode < 2)
		{
			//randomly pick a number of phages from total phage size, with probability qiB * N
			if (binomial == false)
			{
		...
		{
			/*coinfection*/
			//if infection mode = 2, use Withcoinfection.cpp and Withcoinfection.h instead.
		}
		...
		if (migration_mode == 0)
		{
			...
			for (k = 0; k < migration_number; k++)
			{
				...
				a = total_phage_index;
				//work out vector index of selected phage out of total_phage_size: (*demesP[j])[b] 
				for (j = 0; j < X; j++)
				{
					a = a - (*demesP[j]).size();
					if (a <= 0)
		...
		for (j = 0; j < X; j++)
		{
			for (w = 0; w < (*demesB[j]).size(); w++)
			{
				...
				{
					//create burst_size number of new phages
