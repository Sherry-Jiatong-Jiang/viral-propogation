...
public:
  ...
  bool infected;
  bool lysed;
  Bacterium(): label(0), infected(false), lysed(false), t_before_lysis(tao) {}
};
...
public:
  ...
  Ptrlist phageNbrhd; 
  vector<classdesc::ref<Bacterium> > Bacterium;
  Ptrlist BacteriumNbrhd;
  ...
  Cell(unsigned x_, unsigned y_):
    x(x_), y(y_), phage=vector<classdesc::ref<Phage> >(), Bacterium= vector<classdesc::ref<Bacterium> >  {} //Bacterium initialization?????
    
  void addphage(const Phage& b) {
    phage.resize(1); *phage[0]=b;
    phage[0]->cellID=begin()->ID; 
  }
  void removeBacterium(const Bacterium& b) {
    Bacterium.resize(1); *Bacterium[0]=b;
    Bacterium[0]->cellID=begin()->ID; 
  }
...
public:
  ...
  void removeBacteriums(TCL_args args);
  void movephages();
  //void birthdeath();
  void drawphages(TCL_args args);
  void drawBacteriums(TCL_args args);
